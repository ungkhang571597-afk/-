...
OUT_ROOT = ROOT / "output" / "minimal-resume-audit-2026-06-26"
    ...
    return page.evaluate(
        """() => {
          ...
          return {
            ...
            fontBody: getComputedStyle(body).fontFamily,
            fontH1,
            resources
          };
        }"""
...
def save_contact_sheet(variant_dir: Path, variant: str) -> Path:
    ...
    sheet = Image.new("RGB", (cols * (thumb_w + pad) + pad, rows * (thumb_h + 46 + pad) + pad), "#f1eadf")
    ...
    for index, shot in enumerate(shots):
        ...
        frame = Image.new("RGB", (thumb_w, thumb_h), "#161310")
        frame.paste(img, ((thumb_w - img.width) // 2, (thumb_h - img.height) // 2))
        sheet.paste(frame, (x, y))
        label = shot.stem.replace("__", " / ")
        draw.text((x, y + thumb_h + 7), label[:31], fill="#241b16", font=font)
